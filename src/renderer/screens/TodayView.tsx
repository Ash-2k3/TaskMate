...
export default function TodayView() {
  ...
  const hasToday = useReflectionStore((s) => s.hasToday);
  const checkHasToday = useReflectionStore((s) => s.checkHasToday);
  ...
  const PRIORITY_CONFIG = {
    high:   { label: 'High',   labelClass: 'text-rose-400',  borderClass: 'border-l-2 border-rose-400/60',  Icon: Flame   },
    medium: { label: 'Medium', labelClass: 'text-amber-400', borderClass: 'border-l-2 border-amber-400/60', Icon: Minus   },
    low:    { label: 'Low',    labelClass: 'text-sky-400/80', borderClass: 'border-l-2 border-sky-400/40',  Icon: ArrowDown },
  } as const;
          ...
          return (
            <div key={priority} className="mb-1">
              <div className="flex items-center gap-1.5 px-1 pt-3 pb-1.5">
                <cfg.Icon className={`h-3.5 w-3.5 ${cfg.labelClass}`} />
